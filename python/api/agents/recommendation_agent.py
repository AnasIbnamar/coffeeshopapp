...
class RecommendationAgent():
    ...
    def get_apriori_recommendation(self,products,top_k=5):
        ...
        #Sort recommendations list by "confidence"
    ...
    def get_recommendation_from_order(self,messages,order):
        ...
        system_prompt = """
        You are a helpful AI assistant for a coffee shop application which servers drinks and pasteries.
        your task is to recommend items to the user based on their input message. And respond in a friendly but consice way. And put it an ordered list with a very small description
    ...
    def get_response(self,messages):
        ...
        if recommendations == []:
            return {"role":"assistant","content":"Sorry, I can't help you with that recommendation. Can I help you with something else"}

        # Respond to user
        recommendations_str = ", ".join(recommendations)
        system_prompt = f"""
        You are a helpful AI assistant for a coffee shop application which serves drinks and pasteries.
        you task is to recommend items to the user based on their input message. And respond in a friendly but consice way. And put it an unordered list with a very small description.

        I will provide what items you should recommend to the user based o their order in the user message.
        """

        prompt = f"""
        {messages[-1]['content']}

        Please recommend me those items exacly: {recommendations_str}
        """
